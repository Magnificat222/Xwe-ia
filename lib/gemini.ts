// Pinned to a current, stable, production-ready Flash model. NOT using the
// "-latest" alias: Google documents it as pointing to an EXPERIMENTAL model
// with tighter rate limits, unsuitable for production. Google does
// periodically retire pinned model IDs for new API keys — if this ever
// 404s, check https://ai.google.dev/gemini-api/docs/models for the current
// stable name and swap it in here.
    ...
    const res = await fetch(`${GEMINI_URL}?key=${apiKey}`, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          // Some Gemini models "think" by default, which can eat the whole
